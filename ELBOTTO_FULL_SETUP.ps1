...
# ELBOTTO_FULL_SETUP.ps1 — one-click hygiene + venv + tools + menu
$ErrorActionPreference = "Stop"

function Ensure-Venv {
  if (-not (Test-Path ".\.venv\Scripts\python.exe")) {
    try { py -3.11 -m venv .venv } catch {}
    if (-not (Test-Path ".\.venv\Scripts\python.exe")) { try { py -3 -m venv .venv } catch {} }
    if (-not (Test-Path ".\.venv\Scripts\python.exe")) { python -m venv .venv }
  }
  if (-not (Test-Path ".\.venv\Scripts\python.exe")) { throw "Nie udało się utworzyć .venv" }
  & .\.venv\Scripts\python.exe -m pip install --upgrade pip setuptools wheel
  try { & .\.venv\Scripts\pip install -e ".[dev]" } catch {}
  & .\.venv\Scripts\pip install pandas matplotlib websockets pre-commit
  try { & .\.venv\Scripts\pre-commit install } catch {}
}

# Minimal GUI runner
Set-Content -Path "run_gui.bat" -Value @"
@echo off
chcp 65001 >nul
...
# Minimal GUI (Control + live label)
$gui = @"
...
"@
Set-Content -Path "gui_standalone.py" -Value $gui -Encoding UTF8

# Minimal LIVE placeholder (so menu działa nawet bez prawdziwego live)
$live = @"
print('[LIVE] Placeholder. Tu będzie proces live_allinone.py (shadow).')
"@
if (-not (Test-Path "live_allinone.py")) { Set-Content -Path "live_allinone.py" -Value $live -Encoding UTF8 }

# Analyzer placeholder
$an = @"
print('[ANALYZER] Placeholder. Tu będzie analiza equity/feature.')
"@
if (-not (Test-Path "tools")) { New-Item -ItemType Directory -Force -Path "tools" | Out-Null }
if (-not (Test-Path "tools\analyze_elbotto_run.py")) { Set-Content -Path "tools\analyze_elbotto_run.py" -Value $an -Encoding UTF8 }
...
function Menu {
  Write-Host ""
  Write-Host "=========== ELBOTTO MENU ==========="
  Write-Host "1) GUI (Control)"
  Write-Host "2) LIVE (placeholder)"
  Write-Host "3) Analyzer (placeholder)"
  Write-Host "q) Exit"
  $c = Read-Host "Choice"
  switch ($c) {
    "1" { & .\.venv\Scripts\python.exe .\gui_standalone.py; Menu }
    "2" { & .\.venv\Scripts\python.exe .\live_allinone.py;   Menu }
    "3" { & .\.venv\Scripts\python.exe .\tools\analyze_elbotto_run.py; Menu }
    "q" { return }
    default { Menu }
  }
}
