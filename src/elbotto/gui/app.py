...
class DashboardApp:
                ...
                rows.append(
                    {
                        ...
                        "reinvest_allocation_usd": trade.reinvest_allocation,
                        "fee_usd": trade.fee,
    ...
    def render(self) -> str:
        ...
        return (
            ...
            "<table><thead><tr><th>Para</th><th>Transakcji</th><th>Kapitał całkowity (USD)</th><th>Kapitał handlowy (USD)</th>"
            "<th>Oszczędności spot (USD)</th><th>Zysk netto (USD)</th><th>Śr. pozycja (USD)</th><th>Maks. pozycja (USD)</th><th>Maks. obsunięcie (USD)</th><th>Udział spot (%)</th></tr></thead>"
            ...
            "<th>Wartość (USD)</th><th>PNL (USD)</th><th>Alokacja spot (USD)</th><th>Reinwestycja (USD)</th><th>Prowizja (USD)</th><th>Pewność</th><th>Tryb</th></tr></thead>"
    ...
    def _render_symbol_row(self, symbol: str, report: BacktestReport) -> str:
        ...
        return (
            ...
            f"<td>{metrics.get('trading_capital', metrics.get('final_equity', 0.0)):.2f}</td>"
            f"<td>{metrics.get('spot_saved', 0.0):.2f}</td>"
            ...
            f"<td>{metrics.get('spot_ratio', 0.0) * 100:.2f}</td>"
            "</tr>"
        )
    ...
    def _render_trade_row(self, row: Dict[str, float | str]) -> str:
        return (
            ...
            f"<td>{row['reinvest_allocation_usd']:.4f}</td>"
    ...
    def _render_controls(self) -> str:
        ...
        return (
            (
                "<p>Kapitał całkowity: <strong>{capital:.2f} USD</strong>, Kapitał handlowy: <strong>{trading:.2f} USD</strong>, "
                "Oszczędności spot: <strong>{spot:.2f} USD</strong>, Udział spot: <strong>{spot_ratio:.2f}%</strong>. "
                "Zysk netto: <strong>{pnl:.2f} USD</strong>, Średnia pozycja: <strong>{avg_pos:.2f} USD</strong>. "
                "Niewykorzystane okazje (modelowe): <strong>{missed}</strong>, uniknięte straty: <strong>{avoided}</strong>.</p>"
            ).format(
                capital=metrics["capital"],
                trading=metrics["trading"],
                spot=metrics["spot"],
                spot_ratio=metrics["spot_ratio"] * 100,
                pnl=metrics["total_pnl"],
                avg_pos=metrics["avg_notional"],
                missed=metrics["counterfactual_opportunities"],
                avoided=metrics["counterfactual_avoided"],
            )
            + "<p>Możesz zmieniać parametry w locie, m.in. <code>decision_threshold</code>, <code>max_position</code>, "
            "<code>profit_spot_ratio</code>, <code>min_reserve_ratio</code>, <code>probe_ratio</code>, <code>probe_confidence</code>, "
            "<code>strong_signal_threshold</code>, <code>strong_signal_multiplier</code> oraz limity ryzyka (np. <code>risk.max_vpin</code>). "
            "Przykład: <code>app.manual_update(profit_spot_ratio=0.6, strong_signal_multiplier=1.8)</code>.</p>"
        )
    ...
    def _render_auto_summary(self) -> str:
        ...
        for evaluation in self.auto_result.evaluations:
            rows.append(
                ...
                f"<td>{evaluation.profit_spot_ratio:.2f}</td>"
                f"<td>{evaluation.strong_signal_multiplier:.2f}</td>"
                f"<td>{evaluation.probe_ratio:.2f}</td>"
                f"<td>{evaluation.total_pnl:.4f}</td>"
                f"<td>{evaluation.max_drawdown:.4f}</td>"
                f"<td>{evaluation.trading_capital:.2f}</td>"
                f"<td>{evaluation.spot_saved:.2f}</td>"
                "</tr>"
            )
        table = (
            "<table><thead><tr><th>Próg</th><th>Max pozycja</th><th>Training ratio</th><th>Spot ratio</th><th>Moc sygnału</th><th>Sonda</th><th>PNL</th><th>Max DD</th><th>Kapitał handlowy</th><th>Spot</th></tr></thead>"
            f"<tbody>{''.join(rows)}</tbody></table>"
        )
        return (
            "<p>Automat wybrał konfigurację: "
            f"próg={best.decision_threshold:.3f}, max_position={best.max_position:.2f}, training_ratio={best.training_ratio:.2f}, "
            f"profit_spot_ratio={best.profit_spot_ratio:.2f}, strong_signal_multiplier={best.strong_signal_multiplier:.2f}, "
            f"probe_ratio={best.probe_ratio:.2f}.<br/>Łączny wynik punktacji: {self.auto_result.best_score:.4f}</p>"
            + table
        )
    ...
    def _build_grid(self, extra_grid: int) -> Dict[str, Iterable[float]]:
        """Buduje rozszerzoną siatkę parametrów do auto-tuningu."""
    ...
    def _aggregate_metrics(self) -> Dict[str, float]:
        ...
        for report in self.reports.values():
            ...
            spot += metrics.get("spot_saved", 0.0)
            trading_capital += metrics.get("trading_capital", metrics.get("final_equity", 0.0))
            counterfactual_gain += metrics.get("counterfactual_gain", 0.0)
            counterfactual_opportunities += metrics.get("counterfactual_opportunities", 0)
            counterfactual_avoided += metrics.get("counterfactual_avoided", 0)
        avg_notional = avg_notional / trade_count if trade_count else 0.0
        total_equity = capital if capital else trading_capital + spot
        spot_ratio = spot / total_equity if total_equity else 0.0
        return {
            "capital": capital,
