...
class StrategyConfig:
    ...
    profit_spot_ratio: float = 0.5
    min_reserve_ratio: float = 0.2
    probe_ratio: float = 0.15
    probe_confidence: float = 0.45
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.profit_spot_ratio <= 1:
            raise ValueError("profit_spot_ratio musi być w przedziale [0,1]")
        if not 0 <= self.min_reserve_ratio < 1:
            raise ValueError("min_reserve_ratio musi być w [0,1)")
        if not 0 < self.probe_ratio <= 1:
            raise ValueError("probe_ratio musi być w (0,1]")
        if not 0 <= self.probe_confidence < 1:
            raise ValueError("probe_confidence musi być w [0,1)")
        if not 0 <= self.uncertainty_margin < 0.5:
            raise ValueError("uncertainty_margin musi być w [0,0.5)")
        if not 0 < self.strong_signal_threshold <= 1:
            raise ValueError("strong_signal_threshold musi być w (0,1]")
        if self.strong_signal_multiplier < 1:
            raise ValueError("strong_signal_multiplier musi być >= 1")
        if self.decision_threshold - self.uncertainty_margin <= 0:
            raise ValueError("uncertainty_margin nie może przekraczać decision_threshold")
